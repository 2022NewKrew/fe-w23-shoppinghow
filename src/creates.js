...
function createsHotDealHtml() {
  ...
  container.innerHTML = Array(10).fill(0).reduce( (html, item) => html+hotdealItemTpl, ``);
}
...
function createItemsHtml(){
  ...
  const itemIds=Array.from(Array(5).keys());
  const themeItemData=itemIds.map((itemId)=>(itemData[itemId]));
...
function createRecentItems(container, itemIds){
  container.innerHTML=Object.keys(itemIds).map((itemId)=>(
    `<div class="recent-item"><img src="${itemData[itemId].imageSrc}">
    </img></div>`
  )).join("");
}
