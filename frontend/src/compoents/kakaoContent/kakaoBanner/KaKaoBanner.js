...
export default class KaKaoBanner extends Component {
  template() {
    ...
    <div class="cont_event">
        ...
        <div ${getTarget(TARGET_SELECTOR.TARGET_SLIDE_BANNER)} class="#banner_mileage evt_slide _GL">
        </div>
    </div>
    <div ${getTarget(TARGET_SELECTOR.TARGET_MALL_EVENT_LIST)} id="mallEventList" class="#banner_mallevent cont_item cont_top _GL">
    </div>
    `;
  }

  mounted() {
    const $slideBanner = this.$target.querySelector(getTargetSelector(TARGET_SELECTOR.TARGET_SLIDE_BANNER));
